...
class CSVParser {
    static func fetchCountryAccessData() async throws -> [String: [CountryAccess]] {
        ...
        print("Загружено \(countryAccessDict.count) ключей в countryAccessDict")
        if let ruData = countryAccessDict["RU"] {
            print("Для RU найдено \(ruData.count) записей")
            print("Пример записи для RU: \(ruData.first?.destination) - \(ruData.first?.requirement)")
        } else {
            print("Ключ RU не найден в countryAccessDict")
        }
